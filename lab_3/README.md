...
2. Указываю:
   1. `VPC ID`: выбираю мою сеть `student-vpc-k02`
   2. `Subnet name`: `public-subnet-k02`
...
2. Указываю:
   1. `VPC ID`: выбираю мою сеть `student-vpc-k02`
   2. `Subnet name`: `private-subnet-k02`
...
2. Указываю:
   1. `Name tag`: `public-rt-k02`
   2. `VPC`: `student-vpc-k02`
   3. Нажимаю `Create route table`
   4. Перехожу на вкладку `Routes` и нажимаю `Edit routes` → `Add route`.
   5. Заполняю:
      1. `Destination`: 0.0.0.0/0
      2. `Target`: выбираю Internet Gateway (`student-igw-k02`).
...
Таблицу маршрутов нужно привязать к подсети, чтобы именно эта подсеть начала использовать указанные маршруты. Без ассоциации подсеть будет применять основную таблицу маршрутов, и созданный маршрут в Internet Gateway не будет работать.
   ...
   8. Отмечаю `public-subnet-k02` и нажимаю `Save associations`.
...
2. Указываю:
   1. `Name tag`: `private-rt-k02`
   2. `VPC`: `student-vpc-k02`
   3. Нажимаю `Create route table`.
3. Перехожу на вкладку `Subnet associations` → `Edit subnet associations`.
4. Отмечаю `private-subnet-k02` и нажимаю `Save associations`.
...
2. Указываю:
   1. `Name tag`: `nat-gateway-k02`
   2. `Subnet`: выбираю публичную подсеть (`public-subnet-k02`)
...
1. Возвращаюсь в `Route Tables` и выбираю `private-rt-k02`.
2. Перехожу на вкладку `Routes` и нажимаю `Edit routes` → `Add route`.
3. Заполняю:
   1. `Destination`: `0.0.0.0/0`
   2. `Target`: выбираю NAT Gateway (`nat-gateway-k02`).
...
2. Указываю:
   1. `Security group name`: `web-sg-k02`
   2. `Description`: `Security group for web server`
   3. `VPC`: выбираю мою VPC (`student-vpc-k02`)
...
4. Создаю еще две Security Groups:
   1. `bastion-sg-k02` для bastion host с разрешением входящего трафика на порт `22` (SSH) только из моего IP-адреса.

![alt text](images/15.png)

   3. `db-sg-k02` для базы данных с разрешением входящего трафика:
      1. Тип: `MySQL/Aurora`, Протокол: `TCP`, Порт: `3306`, Источник: `web-sg-k02` (разрешаю доступ только с веб-сервера)
      2. Тип: `MySQL/Aurora`, Протокол: `TCP`, Порт: `3306`, Источник: `bastion-sg-k02` (разрешаю доступ только с bastion host)
      3. Тип: `SSH`, Протокол: `TCP`, Порт: `22`, Источник: `bastion-sg-k02` (разрешаю доступ только с bastion host)
...
- _Веб-сервер_ (`web-server-maria`) - в публичной подсети, доступен из Интернета по HTTP.
- _Сервер базы данных_ (`db-server-maria`) - в приватной подсети, недоступен напрямую извне.
- _Bastion Host_ (`bastion-host-maria`) - в публичной подсети, для безопасного доступа к приватным ресурсам.
...
- Ключ доступа (Key Pair): создаю новый ключ `student-key-k02` и скачиваю его.
- Хранилище: оставляю по умолчанию (8 ГБ).
- Теги: добавляю тег `Name` с соответствующим именем инстанса.

Для `web-server-maria`:

![alt text](images/17.png)

1. Выбираю сеть `VPC`: `student-vpc-k02`
2. Подсеть `Subnet`: `public-subnet-k02`
3. `Auto-assign Public IP`: `Enable`
4. `Security Group`: выбираю `web-sg-k02`
...
1. Выбираю сеть `VPC`: `student-vpc-k02`
2. Подсеть `Subnet`: `private-subnet-k02`
3. `Auto-assign Public IP`: `Disable`
4. `Security Group`: выбираю `db-sg-k02`
...
2. Подсеть `Subnet`: `public-subnet-k02`
3. `Auto-assign Public IP`: `Enable`
4. `Security Group`: выбираю `bastion-sg-k02`
...
2. Нахожу публичный IP-адрес `web-server-maria` и открываю его в браузере. Я должна увидеть страницу с информацией о PHP.
...
4. Проверяю подключение к интернету с `bastion-host-maria` выполнив `ping`:
...
У пользователя нет прямого доступа к приватной подсети, но он может подключиться к `db-server-maria` через `bastion-host-maria`.

Использую SSH Agent Forwarding для упрощения доступа:

1. На моей локальной машине выполнила. Это запустило SSH Agent и добавило мой приватный ключ в агент:

   ```bash
   eval "$(ssh-agent -s)"
   ssh-add student-key-k02.pem
   ```
...
`-A` включает SSH Agent Forwarding — механизм, который позволяет использовать локальный приватный ключ при подключении к удалённым серверам через промежуточный узел (bastion-host).<br>
`-J` означает ProxyJump — соединение через "jump-host".

3. Обновляю систему на `db-server-maria`:
   ```bash
   sudo dnf update -y
   ```
...
6. Выхожу из MySQL и затем из `db-server-maria` и `bastion-host-maria`.
7. На локальном компьютере, завершаю работу с SSH Agent:
   ```bash
   ssh-agent -k
   ```
