...
1. **Создаю EC2-инстанс в VPC:**
   ...
   - **Настройка сети:**
     ...
     - VPC: выбираю созданную ранее `project-vpc-02`
...
- **Название:** `project-auto-scaling-group-maria`
- **Launch template:** выбрала свой шаблон `project-launch-template-maria`
...
- указала свою **Target Group (`project-target-group-maria`)**
...
> К сожалению мой браузер не пропускал мои попытки открыть адрес, поэтому был запущен скрипт, который подтвердил полную работу веб-сервера.

![alt text](images/14.png)

3. Затем несколько раз обновила страницу, чтобы посмотреть, как распределяется трафик.

> Так же из-за проблем запустила скрипт, который имитировал тест.
...
> Так же имитировала нагрузку через скрипт.
