...
3. Выбрала регион EU (Stockholm) `eu-north-1`.

> В процессе возникла проблема с регистрацией, поэтому был создан пользователь для меня, другим студентом и предоставлен доступ, для выполнения лабораторных.
...
3. Создала IAM пользователя с именем **cloudstudent**, привязала пользователя к группе `Admins` и разрешила пользователю доступ в AWS Management Console.
...
> Были проделаны все шаги для демонстрации, но бюджет был уже установлен владельцем до меня.
...
![alt text](images/17.png)
    4.  _Key pair_. Была создана новая пара `maria-keypair` для безопасного входа на сервер по SSH и сохранен файл в расширении `.pem`.
...
3. Открыла в браузере http://13.60.228.229 и вижу, что все работает.
...
3. Копирование HTML-файлов на сервер с помощью scp
   ```bash
    scp -i "C:\cloud\lab_2\maria-keypair.pem"index.html ec2-user@13.60.228.229:/tmp
    scp -i "C:\cloud\lab_2\maria-keypair.pem"about.html ec2-user@13.60.228.229:/tmp 
    scp -i "C:\cloud\lab_2\maria-keypair.pem"contact.html ec2-user@13.60.228.229:/tmp
   ``` 
> Возникли проблемы с доступом, но было решено все через копирование файлов в home и после на самом сервере файлы были перемещены в нужное место.
...
5. Открытие в браузере URL: `http://13.60.228.229`, `http://13.60.228.229/about.html`, `http://13.60.228.229/contact.html` и наглядная демонстрация, что сайт работает.
